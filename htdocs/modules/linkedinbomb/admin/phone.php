<?php
	...
	$op = isset($_REQUEST['op'])?$_REQUEST['op']:"emailss";
			...
			switch ($fct) {
				...
				case 'list':
					...
					if (count($skills_profile_ids))
			   		...
			   		$pagenav = new XoopsPageNav($profiles_phones_handler->getCount($criteria), $limit, $start, 'start', 'limit='.$limit.'&sort='.$sort.'&order='.$order.'&op='.$op.'&fct='.$fct.'&filter='.$filter.'&fct='.$fct.'&filter='.$filter);
					$GLOBALS['xoopsTpl']->assign('pagenav', $pagenav->renderNav());
			   		...
			   		if ($phones = $profiles_phones_handler->getObjects($criteria, true)) {
			   			foreach($phones as $phone_id => $phone) {
			   				$GLOBALS['xoopsTpl']->append('phones', $phone->toArray(true));
			   			}
			   		}
					$GLOBALS['xoopsTpl']->display('db:linkedinbomb_cpanel_phones_list.html');
			   		break;
			}
			break;
	}
