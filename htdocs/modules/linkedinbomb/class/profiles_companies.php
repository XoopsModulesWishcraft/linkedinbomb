 ...
 * @package kernel
 */
class LinkedinbombProfiles_companies extends XoopsObject
{

    function __construct($id = null)
    {
        $this->initVar('companies_id', XOBJ_DTYPE_INT, null, false);
		...
		if ($id>0) {
			$handler = new LinkedinbombProfiles_companiesHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombProfiles_companies')) {
    ...
    function getName() {
    	return $this->getVar('company_id').', '.$this->getVar('position_id').', '.$this->getVar('companies_id');
    }
    ...
    function toArray() {
    	...
    	if ($ret['company_id']>0) {
    		...
    		$company = $companies_handler->get($ret['company_id']);
    		$ret['company'] = $company->toArray();
    	}
    	if ($ret['position_id']>0) {
    		$profiles_postions_handler = xoops_getmodulehandler('profiles_positions', 'linkedinbomb');
    		$position = $profiles_postions_handler->get($ret['position_id']);
    		$ret['position'] = $position->toArray();
    	}
...
* @package kernel
*/
class LinkedinbombProfiles_companiesHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_profiles_companies', 'LinkedinbombProfiles_companies', "companies_id", "company_id");
    }
