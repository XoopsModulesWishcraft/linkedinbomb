 ...
 * @package kernel
 */
class LinkedinbombLocations extends XoopsObject
{
    ...
    function __construct($id = null)
    {
        ...
        $this->initVar('contact-info_ids', XOBJ_DTYPE_ARRAY, array(), false);
		...
		if ($id>0) {
			$handler = new LinkedinbombLocationsHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombLocations')) {
    ...
    function toArray() {
    	...
    	if (isset($ret['contact-info_ids'])) {
	    	if (count($ret['contact-info_ids'])>0) {
	    		$contact_info_handler = xoops_getmodulehandler('contact_info', 'linkedinbomb');
	    		foreach($ret['contact-info_ids'] as $contact_info_id) {
			        if ($contact_info = $contact_info_handler->get($contact_info_id)) {
				    	$ret['contact-info'][$contact_info_id] = $contact_info->toArray();
				    }
	    		}	  
	    	}
    	}
    	if ($ret['address_id']>0) {
    		...
    		if ($address = $addresses_handler->get($ret['address_id'])) {
			    $ret['address'] = $address->toArray();
    		}	  
    	}
    	if ($ret['country_id']>0) {
    		$countries_handler = xoops_getmodulehandler('countries', 'linkedinbomb');
    		if ($country = $countries_handler->get($ret['country_id'])) {
...
* @package kernel
*/
class LinkedinbombLocationsHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_locations', 'LinkedinbombLocations', "location_id", "id");
    }
