 ...
 * @package kernel
 */
class LinkedinbombCountries extends XoopsObject
{

    function __construct($id = null)
    {
        $this->initVar('country_id', XOBJ_DTYPE_INT, null, false);
		$this->initVar('code', XOBJ_DTYPE_TXTBOX, null, false, 6);
		$this->initVar('name', XOBJ_DTYPE_TXTBOX, null, false, 198);		
		$this->initVar('year', XOBJ_DTYPE_TXTBOX, null, false, 4);
        $this->initVar('tld', XOBJ_DTYPE_TXTBOX, null, false, 6);
		...
		if ($id>0) {
			$handler = new LinkedinbombCountriesHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombCountries')) {
...
* @package kernel
*/
class LinkedinbombCountriesHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_countries', 'LinkedinbombCountries', "country_id", "name");
    }
