 ...
 * @package kernel
 */
class LinkedinbombProfiles_providers extends XoopsObject
{

    function __construct($id = null)
    {
        $this->initVar('provider_id', XOBJ_DTYPE_INT, null, false);
        $this->initVar('profile_id', XOBJ_DTYPE_INT, null, false);
        $this->initVar('person_id', XOBJ_DTYPE_INT, null, false);
		$this->initVar('provider-account-id', XOBJ_DTYPE_TXTBOX, null, false, 32);
		$this->initVar('provider-account-name', XOBJ_DTYPE_TXTBOX, null, false, 64);		
        $this->initVar('primary', XOBJ_DTYPE_INT, null, false);
		...
		if ($id>0) {
			$handler = new LinkedinbombProfiles_providersHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombProfiles_providers')) {
    ...
    function toArray($include_profile=false) {
        ...
        if ($include_profile==true) {
    		...
    		if (class_exists('TwitterBombFormSelectCampaigns'))
    			$ret['campaign'] = new TwitterBombFormSelectCampaigns('', $this->getVar('profile_id').'[campaign]', false, false, false, false, 'scheduler');
    	}    	
    	return $ret;
    }
...
* @package kernel
*/
class LinkedinbombProfiles_providersHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_profiles_providers', 'LinkedinbombProfiles_providers', "provider_id", "provider-account-name");
    }
