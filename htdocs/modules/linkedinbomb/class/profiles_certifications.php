 ...
 * @package kernel
 */
class LinkedinbombProfiles_certifications extends XoopsObject
{

    function __construct($id = null)
    {
        $this->initVar('certifications_id', XOBJ_DTYPE_INT, null, false);
        ...
        $this->initVar('number', XOBJ_DTYPE_TXTBOX, null, false, 64);
        $this->initVar('start-date', XOBJ_DTYPE_TXTBOX, null, false, 16);
        $this->initVar('end-date', XOBJ_DTYPE_TXTBOX, null, false, 16);
		...
		if ($id>0) {
			$handler = new LinkedinbombProfiles_certificationsHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombProfiles_certifications')) {
    ...
    function toArray() {
    	...
    	if ($ret['authority_id']>0) {
    		$authorities_handler = xoops_getmodulehandler('authorities', 'linkedinbomb');
    		$authority = $authorities_handler->get($ret['authority_id']);
    		$ret['authority'] = $authority->toArray(); 
    	}
    	return $ret;
    }
...
* @package kernel
*/
class LinkedinbombProfiles_certificationsHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_profiles_certifications', 'LinkedinbombProfiles_certifications', "certifications_id", "name");
    }
