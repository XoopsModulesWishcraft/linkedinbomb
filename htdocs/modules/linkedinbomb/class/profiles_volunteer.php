 ...
 * @package kernel
 */
class LinkedinbombProfiles_volunteer extends XoopsObject
{

    function __construct($id = null)
    {
        $this->initVar('volunteer_id', XOBJ_DTYPE_INT, null, false);
		...
		$this->initVar('role', XOBJ_DTYPE_TXTBOX, null, false, 128);		
        $this->initVar('organization_id', XOBJ_DTYPE_INT, null, false);
		...
		if ($id>0) {
			$handler = new LinkedinbombProfiles_volunteerHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombProfiles_volunteer')) {
    ...
    function toArray() {
    	...
    	if ($ret['organization_id']>0) {
    		$organization_handler = xoops_getmodulehandler('organization', 'linkedinbomb');
    		if ($organization = $organization_handler->get($ret['organization_id'])) {
			    $ret['organization'] = $organization->toArray();
    		}	  
    	} 
    	return $ret;
    }
...
* @package kernel
*/
class LinkedinbombProfiles_volunteerHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_profiles_volunteer', 'LinkedinbombProfiles_volunteer', "address_id", "street1");
    }
