 ...
 * @package kernel
 */
class LinkedinbombProfiles_patents_status extends XoopsObject
{
    ...
    function __construct($id = null)
    {
		...
		$this->initVar('name', XOBJ_DTYPE_ENUM, 'Application', false, false, false, false, array('Application','Patent'));
		...
		if ($id>0) {
			$handler = new LinkedinbombProfiles_patents_statusHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombProfiles_patents_status')) {
...
* @package kernel
*/
class LinkedinbombProfiles_patents_statusHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_profiles_patents_status', 'LinkedinbombProfiles_patents_status', "status_id", "name");
    }
