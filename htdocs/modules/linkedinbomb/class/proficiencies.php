 ...
 * @package kernel
 */
class LinkedinbombProficiencies extends XoopsObject
{
    ...
    function __construct($id = null)
    {
		...
		if ($id>0) {
			$handler = new LinkedinbombProficienciesHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombProficiencies')) {
    ...
    function getName() {
    	return $this->getVar('name').' - '.$this->getVar('level');
    }
...
* @package kernel
*/
class LinkedinbombProficienciesHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_proficiencies', 'LinkedinbombProficiencies', "proficiency_id", "name");
    }
