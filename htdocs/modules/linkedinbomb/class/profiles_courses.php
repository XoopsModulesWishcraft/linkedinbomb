 ...
 * @package kernel
 */
class LinkedinbombProfiles_courses extends XoopsObject
{

    function __construct($id = null)
    {
        $this->initVar('courses_id', XOBJ_DTYPE_INT, null, false);
		...
		if ($id>0) {
			$handler = new LinkedinbombProfiles_coursesHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombProfiles_courses')) {
    ...
    function getName() {
    	return $this->getVar('name').' ('.$this->getVar('id').') - '.$this->getVar('number');
    }
...
* @package kernel
*/
class LinkedinbombProfiles_coursesHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_profiles_courses', 'LinkedinbombProfiles_courses', "courses_id", "name");
    }
