 ...
 * @package kernel
 */
class LinkedinbombProfiles_patents_inventors extends XoopsObject
{

    function __construct($id = null)
    {
        $this->initVar('inventors_id', XOBJ_DTYPE_INT, null, false);
        $this->initVar('patent_id', XOBJ_DTYPE_INT, null, false);
        $this->initVar('profile_id', XOBJ_DTYPE_INT, null, false);
        $this->initVar('person_id', XOBJ_DTYPE_INT, array(), false);
		$this->initVar('id', XOBJ_DTYPE_TXTBOX, null, false, 32);
		$this->initVar('name', XOBJ_DTYPE_TXTBOX, null, false, 128);
		$this->initVar('inventor_person_id', XOBJ_DTYPE_INT, array(), false);		
		...
		if ($id>0) {
			$handler = new LinkedinbombProfiles_patents_inventorsHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombProfiles_patents_inventors')) {
    ...
    function toArray() {
        ...
        if ($ret['inventor_person_id']>0) {
    		$persons_handler = xoops_getmodulehandler('persons', 'linkedinbomb');
    		if ($person = $persons_handler->get($ret['inventor_person_id'])) {
...
* @package kernel
*/
class LinkedinbombProfiles_patents_inventorsHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_profiles_patents_inventors', 'LinkedinbombProfiles_patents_inventors', "address_id", "street1");
    }
