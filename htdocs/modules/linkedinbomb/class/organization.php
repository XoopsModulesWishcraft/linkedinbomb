 ...
 * @package kernel
 */
class LinkedinbombOrganization extends XoopsObject
{
    ...
    function __construct($id = null)
    {
		...
		if ($id>0) {
			$handler = new LinkedinbombOrganizationHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombOrganization')) {
...
* @package kernel
*/
class LinkedinbombOrganizationHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_organization', 'LinkedinbombOrganization', "organization_id", "name");
    }
