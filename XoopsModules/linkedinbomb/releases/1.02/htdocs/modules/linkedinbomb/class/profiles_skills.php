 ...
 * @package kernel
 */
class LinkedinbombProfiles_skills extends XoopsObject
{

    function __construct($id = null)
    {
        $this->initVar('skills_id', XOBJ_DTYPE_INT, null, false);
		$this->initVar('profile_id', XOBJ_DTYPE_INT, null, false);
		$this->initVar('person_id', XOBJ_DTYPE_INT, null, false);		
		$this->initVar('skill_id', XOBJ_DTYPE_INT, null, false);
        $this->initVar('proficiency_id', XOBJ_DTYPE_INT, null, false);
        $this->initVar('years_id', XOBJ_DTYPE_INT, null, false);
		...
		if ($id>0) {
			$handler = new LinkedinbombProfiles_skillsHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombProfiles_skills')) {
    ...
    function toArray() {
        ...
        if ($ret['skill_id']>0) {
    		...
    		if ($skill = $skills_handler->get($ret['skill_id'])) {
			    $ret['skill'] = $skill->toArray();
    		}	  
    	}
    	if ($ret['proficiency_id']>0) {
    		$proficiencies_handler = xoops_getmodulehandler('proficiencies', 'linkedinbomb');
    		if ($proficiency = $proficiencies_handler->get($ret['proficiency_id'])) {
			    $ret['proficiency'] = $proficiency->toArray();
    		}	  
    	}
    	if ($ret['years_id']>0) {
    		$years_handler = xoops_getmodulehandler('years', 'linkedinbomb');
    		if ($years = $years_handler->get($ret['years_id'])) {
			    $ret['years'] = $years->toArray();
    		}	  
    	}    	
    	return $ret;
    }
...
* @package kernel
*/
class LinkedinbombProfiles_skillsHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_profiles_skills', 'LinkedinbombProfiles_skills', "skills_id", "skill_id");
    }
