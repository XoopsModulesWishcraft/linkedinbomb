 ...
 * @package kernel
 */
class LinkedinbombSkills extends XoopsObject
{
    ...
    function __construct($id = null)
    {
		...
		if ($id>0) {
			$handler = new LinkedinbombSkillsHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombSkills')) {
    ...
    function toArray() {
    	...
    	if (in_array($this->getVar('skill_id'), $_SESSION['skill_ids'])) {
    		$ret['name'] = '<strong><em>'.$ret['name'].'</em></strong>';
    	}
    	return $ret;
    }
...
* @package kernel
*/
class LinkedinbombSkillsHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_skills', 'LinkedinbombSkills', "skill_id", "name");
    }

    function getProfileIDs($criteria) {
    	$ret = array();
    	if ($skills = $this->getObjects($criteria)) {
    		foreach($skills as $skill_id => $skill) {
    			foreach($skill->getVar('profile_ids') as $profile_id) {
    				$ret[$profile_id] = $profile_id;
    			}
    		}
    	}
    	return $ret;
