...
class LinkedinbombFormCheckBoxSkills extends XoopsFormElement
{
    /**
     * Availlable options
     *
     ...
     */
    function LinkedinbombFormCheckBoxSkills($caption, $name, $value = null, $delimeter = '&nbsp;', $criteria = NULL)
    {
        ...
        if (!is_object($criteria))
        	$criteria = new CriteriaCompo(new Criteria('skill_id', 0, '<>'));
        $criteria->setSort('`name`');
        $criteria->setOrder('ASC');
        if ($skills = $skills_handler->getObjects($criteria, true)) {
        	foreach($skills as $skill_id => $skill) {
				$this->addOption($skill_id, $skill->getVar('name'));        		
        	}
        }
    }
