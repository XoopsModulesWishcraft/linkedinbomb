<?php
	...
	$op = isset($_REQUEST['op'])?$_REQUEST['op']:"profiles";
			...
			switch ($fct) {
				...
				case 'list':
					...
					if (isset($_SESSION['skill_ids'])) {
			   		...
			   		$pagenav = new XoopsPageNav($profiles_handler->getCount($criteria), $limit, $start, 'start', 'limit='.$limit.'&sort='.$sort.'&order='.$order.'&op='.$op.'&fct='.$fct.'&filter='.$filter.'&fct='.$fct.'&filter='.$filter);
					$GLOBALS['xoopsTpl']->assign('pagenav', $pagenav->renderNav());
			   		...
			   		if ($profiles = $profiles_handler->getObjects($criteria, true)) {
			   			foreach($profiles as $profile_id => $profile) {
			   				$GLOBALS['xoopsTpl']->append('profiles', $profile->toArray(true));
			   			}
			   		}
					$GLOBALS['xoopsTpl']->display('db:linkedinbomb_cpanel_profiles_list.html');
			   		break;
			}
			break;
	}
