			...
			switch ($fct) {
				...
				case 'list':
					...
					if (count($skills_profile_ids))
			   		...
			   		$criteria->add(new Criteria('`im-account-name`', '%@%', 'NOT LIKE'));
					...
					$GLOBALS['xoopsTpl']->assign('pagenav', $pagenav->renderNav());
			   		...
			   		if ($ims = $profiles_ims_handler->getObjects($criteria, true)) {
			   			foreach($ims as $im_id => $im) {
			   				if (!checkemail($im->getVar('im-account-name')))
			   					$GLOBALS['xoopsTpl']->append('ims', $im->toArray(true));
			   			}
			   		}
					$GLOBALS['xoopsTpl']->display('db:linkedinbomb_cpanel_ims_list.html');
			   		break;
			}
			break;
	}
