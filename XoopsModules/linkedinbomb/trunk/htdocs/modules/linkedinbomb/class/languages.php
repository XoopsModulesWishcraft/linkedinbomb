 ...
 * @package kernel
 */
class LinkedinbombLanguages extends XoopsObject
{
    ...
    function __construct($id = null)
    {
		...
		if ($id>0) {
			$handler = new LinkedinbombLanguagesHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombLanguages')) {
...
* @package kernel
*/
class LinkedinbombLanguagesHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_languages', 'LinkedinbombLanguages', "language_id", "name");
    }
