 ...
 * @package kernel
 */
class LinkedinbombSpecialties extends XoopsObject
{

    function __construct($id = null)
    {
        $this->initVar('specialties_id', XOBJ_DTYPE_INT, null, false);
		$this->initVar('specialty', XOBJ_DTYPE_TXTBOX, null, false, 128);
		...
		if ($id>0) {
			$handler = new LinkedinbombSpecialtiesHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombSpecialties')) {
...
* @package kernel
*/
class LinkedinbombSpecialtiesHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_specialties', 'LinkedinbombSpecialties', "specialties_id", "specialty");
    }
