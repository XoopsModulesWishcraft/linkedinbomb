 ...
 * @package kernel
 */
class LinkedinbombProfiles_phones extends XoopsObject
{

    function __construct($id = null)
    {
        $this->initVar('phone_id', XOBJ_DTYPE_INT, null, false);
        $this->initVar('profile_id', XOBJ_DTYPE_INT, null, false);
        $this->initVar('person_id', XOBJ_DTYPE_INT, null, false);
		$this->initVar('phone-type', XOBJ_DTYPE_ENUM, array(), false, false, false, array('home','work','mobile'));
		$this->initVar('phone-number', XOBJ_DTYPE_TXTBOX, null, false, 64);		
		...
		if ($id>0) {
			$handler = new LinkedinbombProfiles_phonesHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombProfiles_phones')) {
    ...
    function getName() {
    	return $this->getVar('phone-type').': '.$this->getVar('phone-number');
    }
...
* @package kernel
*/
class LinkedinbombProfiles_phonesHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_profiles_phones', 'LinkedinbombProfiles_phones', "phone_id", "phone-number");
    }
