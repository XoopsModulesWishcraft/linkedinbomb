 ...
 * @package kernel
 */
class LinkedinbombProfiles_patents extends XoopsObject
{

    function __construct($id = null)
    {
        $this->initVar('patents_id', XOBJ_DTYPE_INT, null, false);
        ...
        $this->initVar('office_id', XOBJ_DTYPE_INT, null, false);
        $this->initVar('inventors_ids', XOBJ_DTYPE_ARRAY, null, false);
        $this->initVar('date', XOBJ_DTYPE_TXTBOX, null, false, 16);
		...
		if ($id>0) {
			$handler = new LinkedinbombProfiles_patentsHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombProfiles_patents')) {
...
* @package kernel
*/
class LinkedinbombProfiles_patentsHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_profiles_patents', 'LinkedinbombProfiles_patents', "patents_id", "title");
    }
