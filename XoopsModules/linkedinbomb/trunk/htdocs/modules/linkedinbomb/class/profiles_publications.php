 ...
 * @package kernel
 */
class LinkedinbombProfiles_publications extends XoopsObject
{

    function __construct($id = null)
    {
        $this->initVar('publications_id', XOBJ_DTYPE_INT, null, false);
		...
		$this->initVar('title', XOBJ_DTYPE_TXTBOX, null, false, 198);		
		$this->initVar('publisher', XOBJ_DTYPE_TXTBOX, null, false, 128);
        $this->initVar('authors_ids', XOBJ_DTYPE_ARRAY, array(), false);
        $this->initVar('date', XOBJ_DTYPE_TXTBOX, null, false, 14);
		...
		if ($id>0) {
			$handler = new LinkedinbombProfiles_publicationsHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombProfiles_publications')) {
    ...
    function toArray() {
    	...
    	if (count($ret['authors_ids'])>0) {
    		$profiles_publications_authors_handler = xoops_getmodulehandler('profiles_publications_authors', 'linkedinbomb');
    		foreach($ret['authors_ids'] as $author_id) {
		        if ($author = $profiles_publications_authors_handler->get($author_id)) {
			    	$ret['inventor'][$author_id] = $author->toArray();
			    }
    		}	  
    	}  
    	return $ret;
...
* @package kernel
*/
class LinkedinbombProfiles_publicationsHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_profiles_publications', 'LinkedinbombProfiles_publications', "publications_id", "title");
    }
