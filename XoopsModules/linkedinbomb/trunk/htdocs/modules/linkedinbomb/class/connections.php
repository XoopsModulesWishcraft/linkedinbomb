...
class LinkedinbombConnectionsHandler extends XoopsPersistableObjectHandler
{
    ...
    function insert($object, $force = true) {
    	...
    	if ($object->getVar('request_profile_id')>0&&$object->getVar('request_person_id')==0) {
    		...
    		$object->setVar('request_person_id', $profile->getVar('person_id'));
    	} elseif ($object->getVar('request_profile_id')==0&&$object->getVar('request_person_id')>0) {
    		$persons_handler = xoops_getmodulehandler('persons', 'linkedinbomb');
    		$person = $persons_handler->get($object->getVar('request_person_id'));
    		$object->setVar('request_profile_id', $person->getVar('profile_id'));
    	}
