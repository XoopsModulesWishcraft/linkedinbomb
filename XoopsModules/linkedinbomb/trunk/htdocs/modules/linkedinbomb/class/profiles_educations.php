 ...
 * @package kernel
 */
class LinkedinbombProfiles_educations extends XoopsObject
{

    function __construct($id = null)
    {
        $this->initVar('educations_id', XOBJ_DTYPE_INT, null, false);
		...
		$this->initVar('school-name', XOBJ_DTYPE_TXTBOX, null, false, 128);
		$this->initVar('field-of-study', XOBJ_DTYPE_TXTBOX, null, false, 128);
		$this->initVar('start-date', XOBJ_DTYPE_TXTBOX, null, false, 4);
        $this->initVar('end-date', XOBJ_DTYPE_TXTBOX, null, false, 4);		
		$this->initVar('degree', XOBJ_DTYPE_TXTBOX, null, false, 128);
        $this->initVar('activities', XOBJ_DTYPE_TXTBOX, null, false, 255);
        $this->initVar('notes', XOBJ_DTYPE_TXTBOX, null, false, 255);
		...
		if ($id>0) {
			$handler = new LinkedinbombProfiles_educationsHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombProfiles_educations')) {
    ...
    function getName() {
    	return $this->getVar('school-name').', '.$this->getVar('field-of-study').', '.$this->getVar('start-date').' to '.$this->getVar('end-date');
    }
...
* @package kernel
*/
class LinkedinbombProfiles_educationsHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_profiles_educations', 'LinkedinbombProfiles_educations', "educations_id", "school-name");
    }
