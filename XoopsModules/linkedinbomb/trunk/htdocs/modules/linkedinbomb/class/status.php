 ...
 * @package kernel
 */
class LinkedinbombStatus extends XoopsObject
{

    function __construct($id = null)
    {
        $this->initVar('status_id', XOBJ_DTYPE_INT, null, false);
		$this->initVar('code', XOBJ_DTYPE_TXTBOX, null, false, 32);
		...
		if ($id>0) {
			$handler = new LinkedinbombStatusHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombStatus')) {
...
* @package kernel
*/
class LinkedinbombStatusHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_status', 'LinkedinbombStatus', "status_id", "name");
    }
