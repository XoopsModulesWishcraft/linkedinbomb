 ...
 * @package kernel
 */
class LinkedinbombFollowing_companies extends XoopsObject
{

    function __construct($id = null)
    {
        $this->initVar('cf_id', XOBJ_DTYPE_INT, null, false);
		...
		if ($id>0) {
			$handler = new LinkedinbombFollowing_companiesHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombFollowing_companies')) {
    ...
    function getName() {
    	return $this->getVar('person_id').' - '.$this->getVar('company_id');
    }
...
* @package kernel
*/
class LinkedinbombFollowing_companiesHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_following_companies', 'LinkedinbombFollowing_companies', "cf_id", "street1");
    }
