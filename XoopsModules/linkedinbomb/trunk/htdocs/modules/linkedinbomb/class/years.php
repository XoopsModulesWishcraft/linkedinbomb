 ...
 * @package kernel
 */
class LinkedinbombYears extends XoopsObject
{

    function __construct($id = null)
    {
        $this->initVar('years_id', XOBJ_DTYPE_INT, null, false);
		$this->initVar('id', XOBJ_DTYPE_TXTBOX, null, false, 128);
		...
		if ($id>0) {
			$handler = new LinkedinbombYearsHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombYears')) {
...
* @package kernel
*/
class LinkedinbombYearsHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_years', 'LinkedinbombYears', "years_id", "name");
    }
