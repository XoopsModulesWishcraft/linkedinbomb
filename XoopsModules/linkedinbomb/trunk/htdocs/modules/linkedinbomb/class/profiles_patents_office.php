 ...
 * @package kernel
 */
class LinkedinbombProfiles_patents_offices extends XoopsObject
{
    ...
    function __construct($id = null)
    {
		...
		if ($id>0) {
			$handler = new LinkedinbombProfiles_patents_officesHandler($GLOBALS['xoopsDB']);
			$object = $handler->get($id);
			if (is_object($object)) {
				if (is_a($object, 'LinkedinbombProfiles_patents_offices')) {
...
* @package kernel
*/
class LinkedinbombProfiles_patents_officesHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'lib_profiles_patents_offices', 'LinkedinbombProfiles_patents_offices', "office_id", "name");
    }
